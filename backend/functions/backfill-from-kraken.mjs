/**
 * Backfill bot entries from actual Kraken trade history
 * This fetches trades from Kraken and adds them to the appropriate bots
 */
...
// Encryption key from environment or default (must match backend)
const ENCRYPTION_KEY = process.env.ENCRYPTION_KEY || 'default-key-change-in-production';

/**
 * Decrypt API key (matches settingsStore.ts decryption)
 */
...
function normalizePair(krakenPair) {
  // Kraken returns pairs like "XATOMZUSD" or "ATOMUSD"
  // We need to convert to "ATOM/USD"
...
async function backfillFromKraken() {
