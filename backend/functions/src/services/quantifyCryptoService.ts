...
class QuantifyCryptoService {
  private baseUrl = 'https://api.quantifycrypto.com/v1';
  ...
  private getApiKey(): string | null {
    ...
    if (settingsKey) {
      console.log('[QuantifyCrypto] Using API key from settings');
      return settingsKey;
    }

    // Fall back to environment variable
    if (process.env.QUANTIFY_CRYPTO_API_KEY) {
      console.log('[QuantifyCrypto] Using API key from environment');
   ...
   */
  async getEnhancedTrends(limit: number = 20): Promise<any> {
    try {
      const apiKey = this.getApiKey();

      if (!apiKey) {
        console.warn('[QuantifyCrypto] API key not configured, using mock data');
      ...
      console.error('[QuantifyCrypto] Error fetching enhanced trends:', error.message);

      // Fallback to mock data if API fails
      console.log('[QuantifyCrypto] Falling back to mock enhanced trends data');
      return this.getMockEnhancedTrends(limit);
    }
  }
   ...
   */
  private getMockEnhancedTrends(limit: number = 20): any {
    const symbols = [
    ...
    const trends = symbols.slice(0, limit).map((symbol) => {
  ...
  async testConnection(): Promise<any> {
    try {
      ...
      console.error('[QuantifyCrypto] API test failed:', error.message);
      return {
        success: false,
        error: error.message,
      };
