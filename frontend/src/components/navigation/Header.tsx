...
export default function Header() {
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between px-6 transition-colors duration-200 shadow-sm">
      <div className="flex items-center gap-6">
        ...
        {balances.length > 0 && (
          <div className="flex items-center gap-4 pl-4 border-l border-slate-200 dark:border-slate-700">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-gray-400">Portfolio</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-gray-400">24h</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-slate-400 dark:text-gray-400">Stables</p>
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-slate-200 dark:border-slate-700">
          {/* DalyDCA Status */}
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700">
            <Activity className={`h-4 w-4 ${strategyStatus.dca.active ? 'text-emerald-500' : 'text-slate-400'}`} />
            <div className="flex flex-col">
              <p className="text-xs text-slate-400 dark:text-gray-400">DalyDCA</p>
              <p className={`text-xs font-semibold ${strategyStatus.dca.active ? 'text-emerald-500' : 'text-slate-400'}`}>
          ...
          {/* DalyDEPEG Status */}
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-700">
            <Activity className={`h-4 w-4 ${strategyStatus.depeg.active ? 'text-emerald-500' : 'text-slate-400'}`} />
            <div className="flex flex-col">
              <p className="text-xs text-slate-400 dark:text-gray-400">DalyDEPEG</p>
              <p className={`text-xs font-semibold ${strategyStatus.depeg.active ? 'text-emerald-500' : 'text-slate-400'}`}>
      ...
      <div className="flex items-center space-x-4">
        <div className="relative">
          <button
            ...
            className="relative p-2 text-slate-400 dark:text-gray-400 hover:text-slate-600 dark:hover:text-white transition-colors"
          >
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center space-x-2 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
          >
            <User className="h-5 w-5 text-slate-400 dark:text-gray-400" />
            <span className="text-sm text-slate-600 dark:text-gray-300">{user?.username}</span>
          </button>
